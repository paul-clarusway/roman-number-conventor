...
@app.route("/")
def index():
    return render_template("index.html",developer_name='Paul-Instructor')
...
def convert_html():
    if request.method=="POST":
        ...
        if int(number1)<1 or not number1.isdigit() or int(number1)>3999:
            return render_template("convert.html", developer_name='Paul-Instructor')
        else:
            return render_template("convert.html", developer_name='Paul-Instructor', convert=convert(number1),number=number1)
    else:
